...
BOPTFINAL=zeros(3,10); %fila 1 b01, fila 2 b02
ph=0.5;
...
for q=1:max(size(deltagrid))

BOPTFINAL(3,q) = deltagrid(q);

tita1H=1+deltagrid(q);
tita1L=1-deltagrid(q);
tita2H=alpha*(1+deltagrid(q))+(1-alpha)*(1-deltagrid(q));
tita2L=alpha*(1-deltagrid(q))+(1-alpha)*(1+deltagrid(q));
OF1H= cell(1,gr);
OF1L= cell(1,gr);
OF2= zeros(gr,gr);
EC= zeros(gr,gr);
